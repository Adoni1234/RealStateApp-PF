...
public class HomeController : Controller
{
    ...
    // CORREGIDO: el parametro es HomeIndexViewModel para que el binding
    // coincida con los name="Filters.X" que envía la vista
    public async Task<IActionResult> Index(HomeIndexViewModel model)
    {
        ...
        model.Filters ??= new PropertyViewModelFilters();

        var filtersDto = new PropertyFiltersDto
        {
            SelectedPropertyTypeId = model.Filters.SelectedPropertyTypeId,
            Bathrooms = model.Filters.Bathrooms,
            MaxValue = model.Filters.MaxValue,
            MinValue = model.Filters.MinValue,
            Rooms = model.Filters.Rooms,
            ClientId = userId,
            OnlyFavorites = model.Filters.OnlyFavorites,
            PropertyCode = model.Filters.PropertyCode
        };

        model.Properties =
            _mapper.Map<List<PropertyViewModel>>(await _propertyService.GetAllAvailablePropertiesAsync(filtersDto));
        model.PropertyTypes =
            _mapper.Map<List<PropertyTypeViewModel>>(await _propertyTypeService.GetAllAsync());
    ...
    [HttpPost]
    public async Task<IActionResult> ToggleFavorite(int propertyId, HomeIndexViewModel model)
    {
        var userId = User.FindFirstValue(ClaimTypes.NameIdentifier) ?? "";
        ...
        {
            Filters_OnlyFavorites = model.Filters?.OnlyFavorites,
            Filters_PropertyCode = model.Filters?.PropertyCode,
            Filters_SelectedPropertyTypeId = model.Filters?.SelectedPropertyTypeId,
            Filters_MinValue = model.Filters?.MinValue,
            Filters_MaxValue = model.Filters?.MaxValue,
            Filters_Rooms = model.Filters?.Rooms,
            Filters_Bathrooms = model.Filters?.Bathrooms,
        });
    }
